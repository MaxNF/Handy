...
import ru.netfantazii.handy.data.model.BillingObject
import ru.netfantazii.handy.data.model.BillingPurchaseTypes
import ru.netfantazii.handy.data.model.ShopItem
import ru.netfantazii.handy.data.remotedb.SkuList
    ...
    private fun queryPurchaseForPrice(params: SkuDetailsParams) =
        billingRepositoryImpl.getSkuDetails(params)
            .map { skuDetailsList ->
                skuDetailsList.map {
                    BillingObject(it,
                        getBillingItemTypeFromSku(it.sku))
                }
            }
