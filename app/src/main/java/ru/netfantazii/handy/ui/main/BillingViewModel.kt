...
import ru.netfantazii.handy.data.usecases.BillingInteractor
...
class BillingViewModel @Inject constructor(
    ...
    private fun observePurchases() {
        disposables.add(billingInteractor.observePurchases()
            ...
            .subscribe({ shopItem ->
                ...
                _unknownBillingException.value =
                    Event(it)
            }))
    }

    private fun establishConnectionAndGetPricesAndCurrentPremiumStatus() {
        disposables.add(billingInteractor.connectToBillingAndGetPrices()
            .observeOn(AndroidSchedulers.mainThread())
            .retryWhen { errorStream ->
...
//                        t.printStackTrace()
    ...
    fun setCurrentPremiumStatus() {
        if (billingInteractor.isBillingClientReady()) {
            billingInteractor.getCurrentPremiumStatus()?.let {
    ...
    fun launchBillingFlow(activity: Activity, billingObject: BillingObject) {
        val result = billingInteractor.launchBillingFlow(activity, billingObject)
        if (result.responseCode != BillingClient.BillingResponseCode.OK) {
            _billingFlowError.value =
                Event(result.responseCode)
        }
        Log.d(TAG, "launchBillingFlow: ${result.responseCode}")
    }

    fun openGooglePlaySubscriptionSetting() {
        _openSubscriptionSettingsClicked.value =
            Event(Unit)
    }

    fun onOneMonthButtonClick() {
        Log.d(TAG, "onOneMonthButtonClick: ")
        oneMonthBillingObject?.let { _oneMonthButtonClicked.value =
            Event(it)
        }
    ...
    fun onOneYearButtonClick() {
        ...
        oneYearBillingObject?.let { _oneYearButtonClicked.value =
            Event(it)
        }
    }
    ...
    fun onForeverButtonClick() {
        ...
        foreverBillingObject?.let { _foreverButtonClicked.value =
            Event(it)
        }
    }
}
