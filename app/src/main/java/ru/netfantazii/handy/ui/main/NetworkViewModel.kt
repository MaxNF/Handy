...
import ru.netfantazii.handy.data.model.*
import ru.netfantazii.handy.ui.contacts.ContactsClickHandler
import ru.netfantazii.handy.ui.contacts.ContactsStorage
import ru.netfantazii.handy.ui.contacts.DialogClickHandler
import ru.netfantazii.handy.utils.extensions.copyTextToClipboard
...
class NetworkViewModel @Inject constructor(private val remoteRepository: RemoteRepository) :
    ...
    private val _firebaseSignInError = MutableLiveData<Event<Throwable>>()
    val firebaseSignInError: LiveData<Event<Throwable>> = _firebaseSignInError
    ...
    fun signInToFirebase(account: GoogleSignInAccount) {
        ...
        disposables.add(remoteRepository.signInToFirebase(credential)
            ...
            .subscribe({ (first, second) ->
                ...
                _firebaseSignInError.value = Event(it)
                hidePb()
            }))
    }
        ...
        disposables.add(
            when (action) {
                ...
                ContactDialogAction.DELETE -> {
                    throw UnsupportedOperationException("Deletion should be performed in the corresponding method")
                }
            })
    }
    ...
    fun sendCatalog(catalogContent: Map<String, Any>) {
        ...
        disposables.add(remoteRepository.sendCatalog(catalogContent)
            ...
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe {
                _catalogSentSuccessfully.value =
                    Event(catalogName)
                hidePb()
            })
    }
    ...
    fun onDeleteAccountYesClick() {
        ...
        disposables.add(remoteRepository.reauthenticateInFirebase(user.get()!!.credential)
            ...
            .subscribe {
                _accountDeletedSuccessfully.value =
                    Event(Unit)
                doSignOutCompanionOperations()
                hidePb()
            })
    ...
    fun shareSecretCode() {
        _shareSecretCodeClicked.value =
            Event(user.get()?.secret ?: "n/a")
    }
}
